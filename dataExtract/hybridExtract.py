from mixandmatch import hybrid
import os
import time
...
def runGraph189(file1, level):
    graphNum = [1,8,9]
    updateValues=[file1, file1, file1]
    inputSize = os.path.getsize(file1)
    output = hybrid.process3W(updateValues,level)
...
def runGraph2356(file1, file2, file1E, file2E, level):
    ...
    output = hybrid.process4W(updateValues, level)
    outputValues = output[0]
    timeValues = output[1]
    index=0
    for x in outputValues:
...
def runGraph4710(file1, file2, file3, file1E, file2E, file3E, level):
